...
# --- Platform facts -----------------------------------------------------------
#
# What kind of machine this is. Unlike a config namespace these are true before
# anything is published -- a launcher, a standalone script and the frontend
# process all need them -- so the context answers by probing when no override is
# installed. `utils.common` keeps the only real implementation; this is its
# address, not a second copy.
#
# The address is the point. A fact with one home can be moved in one place: a
# reader asks `get_platform().is_sm100`, and a test (or a runtime that knows
# better) says so once instead of patching every module that imported a probe.
...
# Not yes/no facts: the compute capability itself. Same address, same override.
...
class PlatformContext:
    ...
    def __setattr__(self, name: str, value: Any) -> None:
        raise AttributeError(
            ...
            "`get_context().override_platform(...)` so every reader agrees"
        )
...
def describe_kv_events_publisher(server_args: Any) -> Optional[dict]:
    ...
    * the endpoint is not a routable TCP address (inproc:// /
    ...
    resolved_range, _reason = resolve_load_pub_range(
        ...
        dp_size=resolved.dp_size,
        load_publish_endpoint=server_args.load_publish_endpoint,
    )
